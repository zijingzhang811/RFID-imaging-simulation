function [ c, clusters,centroidDist ] = i4block_components ( a, roomSize, voxelSize )
...
%% Adding this part that gives centroid information of all the connected components
c_vec = c(:);

centroidVec = zeros(component_num,3);
elemNum = zeros(component_num,1);

minmaxX = zeros(component_num,2);
minmaxY = zeros(component_num,2);
minmaxZ = zeros(component_num,2);

for component = 1:component_num
    a = c_vec==component;
    elemNum(component) = length(find(a~=0));
    xyzCoordInCluster = xyzVoxelCoord(a,:);
    minmaxX(component,:) = [min(xyzCoordInCluster(:,1)),max(xyzCoordInCluster(:,1))];
    minmaxY(component,:) = [min(xyzCoordInCluster(:,2)),max(xyzCoordInCluster(:,2))];
    minmaxZ(component,:) = [min(xyzCoordInCluster(:,3)),max(xyzCoordInCluster(:,3))];
    centroidVec(component,:) = mean(xyzCoordInCluster,1); % Imp, take mean of each column
end    

clusters.centroid = centroidVec;
clusters.compNum = component_num;
clusters.elemNum = elemNum;
clusters.minmaxX = minmaxX;
clusters.minmaxY = minmaxY;
clusters.minmaxZ = minmaxZ;

% Now you get pairwise distances between all pairs of centroid
centroidDist = (pdist(centroidVec,'euclidean')); % Can use squareform()
